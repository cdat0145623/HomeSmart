...
function AuthPage() {
    ...
    useEffect(() => {
            ...
            callback: async (response) => {
                try {
                    ...
                    if (r.status === 202) {
                        ...
                        navigate("/dang-nhap?tab=login&verifySent=1", {
                            replace: true,
                        });
                        return;
                    }
                    ...
                    if (meData?.ok) {
                        login(meData.data.user, meData.data.token);
                    ...
                    toast.error(
                        `Lỗi Google: ${e?.message || "Không xác định"}`
                    );
                }
            },
            ux_mode: "popup",
        ...
        safeRenderGoogleBtn(
            tab === "login" ? "googleLoginBtn" : "googleSignupBtn"
        );
            ...
            <main className="container-auth mx-auto px-4 py-10" role="main">
                <div className="nh-auth-shell">
                    ...
                    >
                        ...
                        <p className="nh-auth-desc">
                            Đăng nhập hoặc tạo tài khoản để tận hưởng ưu đãi
                            thông minh
                        </p>
                    </header>
                    ...
                    <div
                        className="nh-tabs"
                        role="tablist"
                        aria-label="Chuyển tab đăng nhập/đăng ký"
                    >
                        <button
                            role="tab"
                    ...
                    <div className="nh-card" role="region" aria-live="polite">
                        {tab === "login" ? (
                            <Dangnhap setTab={setTab} />
                        ) : (
                            <Dangki setTab={setTab} />
                        )}
                    </div>
                </div>
            </main>
