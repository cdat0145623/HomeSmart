...
function BrandForm() {
    ...
    useEffect(() => {
        (async () => {
            if (isEdit && search.get("delete") === "1") {
                ...
                if (!ok)
                    return navigate(`/admin/dashboard/brands/${id}`, {
                        replace: true,
                    });
                try {
                    const res = await api(`/admin/brands/${id}`, {
                        method: "DELETE",
                        withCred: true,
                    });
                    if (!res?.ok)
    ...
    function onNameBlur() {
        if (!form.duong_dan_ten_seo && form.ten_thuong_hieu) {
            setForm((s) => ({
                ...s,
                duong_dan_ten_seo: slugify(s.ten_thuong_hieu),
            }));
        }
    }
    ...
    async function onSubmit(e) {
        ...
        try {
            ...
            navigate("/admin/dashboard/brands", { replace: true });
        } catch (err) {
