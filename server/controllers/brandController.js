...
const getAllBrands = async (req, res, next) => {
    try {
        ...
        console.log("Error at getAllBrand api:", e);
    }
};

// GET /admin/brands?page&pageSize&q
async function listBrands(req, res) {
    console.log("req.query query brands;:::", req?.query?.q);
    try {
...
async function getBrand(req, res) {
    try {
        ...
        if (!row)
            return res
                .status(404)
                .json({ ok: false, message: "Không tìm thấy thương hiệu" });
...
async function createBrand(req, res) {
    uploadBrandLogo(req, res, async (err) => {
        if (err) {
            ...
            return res
                .status(400)
                .json({ ok: false, message: "Upload logo thất bại" });
        }
        try {
            ...
            if (!ten)
                return res
                    .status(400)
                    .json({ ok: false, message: "Thiếu tên thương hiệu" });
            ...
            if (!logo)
                return res
                    .status(400)
                    .json({ ok: false, message: "Thiếu logo_thuong_hieu" });
...
async function deleteBrand(req, res) {
    try {
        ...
        const [ref] = await pool.query(
            "SELECT id FROM san_pham WHERE thuong_hieu_id=? LIMIT 1",
            [id]
        );
        if (ref.length) {
            return res.status(400).json({
                ok: false,
                message:
                    "Không thể xóa: đang có sản phẩm tham chiếu thương hiệu này",
            });
        }
        const [r] = await pool.query(
            "DELETE FROM thuong_hieu WHERE id=? LIMIT 1",
            [id]
        );
        if (!r.affectedRows) {
            return res
